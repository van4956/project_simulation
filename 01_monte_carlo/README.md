...
Метод Монте-Карло применяется, когда аналитический расчёт слишком сложен или невозможен.
...
- запускаем много симуляций (но неравномерно, хорошие ветви исследуются чаще, плохие - реже)
...
>
> Го знаменита своей стратегической глубиной и простыми правилами, которые открывают огромное количество комбинаций и стратегий (число возможных позиций больше, чем атомов во Вселенной).
>
> В Го brute-force невозможен, поэтому для эффективной игры нужны вероятностные и обучаемые методы.

Основные архитектура AlphaGo:
...
> AlphaGo (2015–2016) тренировалась на партиях людей, после имела огромную серию self-play, имела раздельные policy и value сети, MCTS который сочетал статистику посещений и предсказания (policy/value), находил наилучший ход. Все это позволило переиграть сильнейших чемпионов Го.
>
...
Алгоритм работает так: берём известные карты (карты игрока и карты на столе), раздаём случайные карты противникам, досдаём оставшиеся карты стола, определяем победителя. Повторяем процесс N раз и считаем процент побед. Чем больше симуляций — тем точнее результат.
...
**`poker_simulation.ipynb`** — Интерактивный Jupyter Notebook с виджетами

Возможности:

- Выбор карт игрока
- Выбор карт стола
- Настройка количества игроков (2-9)
- Настройка количества симуляций (1.000-100.000)
- Визуализация результатов
- Расчёт вероятности в реальном времени
