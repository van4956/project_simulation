...
Это карточная настольная игра для 3-5 игроков. В ней 5 мастей, каждый играет за свою масть, собирает карты в разных зонах, с помощью игровых механик карты перемещаются и удаляются с поля. Побеждает тот, у кого будет большее количество своих карт на последнем ходе.

Более подробные правила в документе `Battle_of_Dimensions_Rules.pdf`

Это игра моего авторства. Идею вынашивали с другом несколько лет. В 2024 при поддержки GPT и Mind, за несколько итераций смогли собрать играбельный прототип, распечатать несколько наборов.
...
Однако это все очень затратно и долго. А так как я программист, то решил посчитать баланс с помощью Python. Скрипты с кодом доступны выше, в папке `/02_battle_dimensions`. Расчеты ниже.
...
- Синяя фракция на втором месте
- чем меньше игроков в партии, тем сильнее перевес Фиолетовой фракции, так как меньшее количесво игроков (и действие) сопротивляется доминации Фиолетовых
